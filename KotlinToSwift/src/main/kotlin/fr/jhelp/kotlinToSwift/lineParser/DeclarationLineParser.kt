...
private val PATTERN_DECLARATION =
    Pattern.compile("((?:private|internal|public)\\s+)?((?:@T|t)ry\\s+)?(?:const\\s+)?(@Weak\\s+)?(va[rl]\\s+)([a-zA-Z][a-zA-Z0-9_]*)(.*)")
private const val GROUP_DECLARATION_PRIVATE = 1
private const val GROUP_DECLARATION_TRY = 2
private const val GROUP_DECLARATION_WEAK = 3
private const val GROUP_DECLARATION_VALR = 4
private const val GROUP_DECLARATION_NAME = 5
private const val GROUP_DECLARATION_SPECIFICATION = 6
...
class DeclarationLineParser : LineParser
{
    override fun parse(trimLine: String): String
    {
        ...
        if (matcher.matches())
        {
            ...
            matcher.group(GROUP_DECLARATION_WEAK)?.let { parsed.append(" weak ") }
