...
private val ABSTRACT_FUNCTION_PATTERN =
    Pattern.compile("(?:(@Throws)\\s+)?((?:(?:override|private|public|internal|open)\\s+)*abstract\\s+fun)(\\s+<[a-zA-Z0-9_, :<>]+>)?(\\s+[a-zA-Z][a-zA-Z0-9_<>.]*\\s*)\\(((?:.|\\n)*)\\)(?:\\s*:\\s*([a-zA-Z][a-zA-Z0-9_<>?, ]*))?$")
...
class FunLineParser : LineParser
{
    ...
    {
        var matcherFun = FUNCTION_PATTERN.matcher(trimLine)
        ...
        matcherFun = ABSTRACT_FUNCTION_PATTERN.matcher(trimLine)
        ...
        if (matcherFun.matches())
        {
            ...
            var declaration = matcherFun.group(GROUP_FUN_DECLARATION).replace("abstract", "open")
            ...
            if (returnSomething)
            {
                parsed.append("return fatal")
            }
            else
            {
                parsed.append("fatalError")
            }

            parsed.append("(\"Abstract method, must override ")
            parsed.append(name)
            parsed.append("\")\n     }")
            return parsed.toString()
        }
